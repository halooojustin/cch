...
export function parseJsonl(filePath: string, knownMtime?: number): SessionInfo | null {
  try {
    ...
    // Skip noise sessions
    const noise = firstMsg.trim().toLowerCase();
    if (
      noise.startsWith("你是一个会话分类助手") ||
      noise.startsWith("你是一个会话历史搜索助手") ||
      noise.startsWith("你是一个开发者工作台的会话分类助手") ||
      noise.startsWith("<local-command-caveat>") ||
      noise === "hi" || noise === "hello" || noise === "hey" ||
      noise === "ok" || noise === "test"
    ) {
      return null;
    }
