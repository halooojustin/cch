...
export function parseJsonl(filePath: string, knownMtime?: number): SessionInfo | null {
  try {
    ...
    // Skip sessions generated by claude -p (cch's own AI calls)
    if (firstMsg.startsWith("你是一个会话分类助手") ||
        firstMsg.startsWith("你是一个会话历史搜索助手") ||
        firstMsg.startsWith("你是一个开发者工作台的会话分类助手")) {
      return null;
    }
